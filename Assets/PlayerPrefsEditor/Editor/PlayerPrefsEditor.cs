...
public class PlayerPrefsEditor : EditorWindow
{
	...
	private PlayerPrefPair[] RetrieveSavedPrefs(string companyName, string productName)
    {
		if(Application.platform == RuntimePlatform.OSXEditor)
		{
			...
			string playerPrefsPath = getOSXPlayerPrefsPath();
	...
	private string getOSXPlayerPrefsPath()
	{
		// Construct the plist filename from the project's settings
		string plistFilename = string.Format("unity.{0}.{1}.plist", PlayerSettings.companyName, PlayerSettings.productName).Replace("&", "_");
